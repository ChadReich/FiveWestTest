    ...
    async with websockets.connect(uri, extra_headers=headers) as websocket:
        ...
        while True:
            try:
                ...
                if update["type"] == "FULL_ORDERBOOK_SNAPSHOT":
                    order_book.process_full_orderbook_snapshot(update)
                elif update["type"] == "FULL_ORDERBOOK_UPDATE":
                    order_book.process_full_orderbook_update(update)
                elif update["type"] == "PONG":
...
if __name__ == "__main__":
    ...
    asyncio.get_event_loop().run_until_complete(connect_to_valr_trade_websocket(api_key, api_key_secret))
    import uvicorn
